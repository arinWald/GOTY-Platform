...
bool SlimeEnemy::Start() {
	...
	pbody = app->physics->CreateCircle(startPos.x, startPos.y, width / 4.5, bodyType::DYNAMIC);
	pbody->ctype = ColliderType::ENEMY;

	pbody->listener = this;

	hitbox = app->physics->CreateRectangleSensor(METERS_TO_PIXELS(pbody->body->GetTransform().p.x), METERS_TO_PIXELS(pbody->body->GetTransform().p.y) - 15, 5, 2, bodyType::STATIC);
	hitbox->ctype = ColliderType::SLIME_HITBOX;
